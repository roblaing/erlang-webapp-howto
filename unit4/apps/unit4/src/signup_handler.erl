...
init(Req0=#{method := <<"POST">>}, State) ->
  ...
  if
    Hash =:= false -> Content = webutil:template(code:priv_dir(unit4) ++ "/signup_form.html", 
                      webutil:html_escape([Name, "Oops, no cookie!", Email])),
                      Req = cowboy_req:reply(200, #{<<"content-type">> => <<"text/html; charset=UTF-8">>}, Content, Req0);
    true ->  Query = pgo:query("INSERT INTO users (id, name, email) VALUES ($1::text, $2::text, $3::text)", 
